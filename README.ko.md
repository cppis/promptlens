...
Prompatic는 Claude Desktop과 Claude Code에서 동작하는 MCP(Model Context Protocol) 서버입니다.
...
> 5분 안에 Prompatic를 설치하고 첫 프롬프트를 분석해봅니다.  
...
Prompatic를 Claude에 연결하는 단계입니다. 개발 중인 소스를 사용하는 방법과 npm 배포판을 사용하는 방법이 있습니다.
...
자동 설정 스크립트를 실행하면 Claude Desktop에 Prompatic가 등록됩니다:
...
정상적으로 등록되면 Claude Desktop 입력창 우측 하단에 MCP 도구 아이콘(🔧)이 나타나고, `prompatic`가 목록에 보입니다.
...
출력에 `prompatic`가 있으면 성공입니다.

---

## Step 3. 첫 프롬프트 분석

Prompatic는 아래 방법 중 하나만 감지하면 **자동으로 호출**됩니다.

### Trigger Rules — Prompatic 자동 호출 조건
...
Prompatic 기능별 토큰 소비량 비교입니다. **모델 선택이 가장 큰 변수**입니다.
...
Prompatic가 자동으로 호출되어 분석 결과를 돌려줍니다:
...
Claude Desktop 대화창에서 `/`를 입력하면 Prompatic 워크플로우 프리셋을 선택할 수 있습니다:
...
| 문서 | 내용 |
|------|------|
| [확장 기능 가이드](docs/3.usage.advanced.md) | 프로젝트 관리, 버전 관리, 시각화, API 모드, 로컬 파일 저장소 |
| [사용 가이드](docs/3.usage.md) | 도구별 파라미터, 시나리오별 사용법, FAQ |
...
| 프로젝트 생성 | `"Prompatic에 '프로젝트명' 프로젝트를 만들어줘"` |
| 활성 프로젝트 설정 | `"'프로젝트명'을 활성 프로젝트로 설정해줘"` |
| 프로젝트 시각화 | `... >> viz` 또는 `... >> 시각화` |
| 프롬프트 비교 | `... >> diff` |

전체 명령 목록은 [확장 기능 가이드](docs/3.usage.advanced.md#전체-명령-모음)를 참고하세요.
